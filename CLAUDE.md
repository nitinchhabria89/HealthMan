...
- **PWA / Add to Home Screen**: `app/manifest.ts` (name/icons/`display: "standalone"`/`start_url: "/dashboard"`), `app/apple-icon.tsx` + `app/icon-192/route.tsx` + `app/icon-512/route.tsx` (all generate PNGs at request time via `next/og`'s `ImageResponse`, recreating the N-mark design in code — no binary image assets checked in), and `app/layout.tsx`'s `metadata.appleWebApp` + `viewport.themeColor` (the `apple-mobile-web-app-capable` meta tag is what makes iOS launch it standalone instead of just opening Safari with a URL bar). `app/icon.svg` (browser tab favicon) is separate and untouched by this.
